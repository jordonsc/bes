...
class MappedRouter : public Router
{
   ...
   private:
    ...
    static void ParseUri(Route&, std::string const&);

    static ActionArgs RouteMatch(PrecachedRoute const& route, std::string const& uri);

    template <class T>
    static T GetNodeValue(YAML::Node&, std::string const& key, T default_value);
};
...
template <class T>
inline T MappedRouter::GetNodeValue(YAML::Node& node, std::string const& key, T default_value)
{
    try {
